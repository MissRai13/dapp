...
const WithdrawRequestCard = ({props,withdrawReq, setWithdrawReq,contractAddress}) => {
  ...
  const withdrawBalance = (reqId) =>{
    setBtnLoader(reqId)
      ...
      const onSuccess = () =>{
        setBtnLoader(false)
        const updatedReq = withdrawReq.map((data)=> data.requestId === props.requestId ? {...data, status:"Completed"} : data)
        setWithdrawReq(updatedReq)
        toastSuccess(`Withdraw successful for request id ${reqId}`)
      }
  ...
  const vote = (reqId) =>{
    setBtnLoader(reqId)
      ...
      const onSuccess = () =>{
        setBtnLoader(false)
        const updatedReq = withdrawReq.map((data)=> data.requestId === props.requestId ? {...data, totalVote:Number(data.totalVote)+1} : data)
        setWithdrawReq(updatedReq)
    ...
    <div className="flex flex-col lg:flex-row">
      ...
      <div className="inner-card my-6 w-full lg:w-3/5">
        ...
        <p className="text-sm font-bold font-sans text-gray-600 w-40 truncate ">{props.reciptant}</p>
        {
            ...
            :
            <button className="withdraw-button" onClick={()=>vote(props.requestId)} disabled={props.status==="Completed"}>
