...
import { getEthereumProvider, groupContributionByProject, groupContributors, projectDataFormatter, withdrawRequestDataFormatter} from "../helper/helper";

const DEFAULT_LOCAL_CONTRACT_ADDRESS = "0x5FbDB2315678afecb367f032d93F642f64180aa3";
const NETWORK_CONTRACT_MAP = {
  31337: process.env.NEXT_PUBLIC_LOCAL_CROWDFUNDING_ADDRESS || DEFAULT_LOCAL_CONTRACT_ADDRESS,
  11155111: process.env.NEXT_PUBLIC_SEPOLIA_CROWDFUNDING_ADDRESS || "",
};

const getCrowdFundingAddress = (networkId) => {
  return NETWORK_CONTRACT_MAP[networkId] || process.env.NEXT_PUBLIC_CROWDFUNDING_ADDRESS || "";
};
...
export const loadWeb3 = async (dispatch) => {
  dispatch(actions.appStatusUpdated({ loading: true, error: "" }));
  const provider = getEthereumProvider() || Web3.givenProvider || "http://localhost:8545";
...
export const loadAccount = async (web3, dispatch) => {
  ...
  dispatch(actions.walletAddressLoaded(walletAddress));
...
export const loadCrowdFundingContract = async(web3,dispatch) =>{
  ...
  dispatch(actions.networkLoaded({
    chainId: network,
    name: getNetworkName(network),
  }));
  const crowdFundingContractAddress = getCrowdFundingAddress(network);
  if (!crowdFundingContractAddress) {
    throw new Error(`Contract address is not configured for network id ${network}`);
  }
  const code = await web3.eth.getCode(crowdFundingContractAddress);
  if (code === "0x") {
    throw new Error(`Contract not found at ${crowdFundingContractAddress}. Deploy it first and update env address.`);
  }
  ...
  dispatch(actions.appStatusUpdated({
    loading: false,
    error: "",
