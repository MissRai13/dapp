    ...
    case types.INCREASE_PROGRESS:
      ...
      var updatedState = state.projects.map(data=>{
        if(data.address === projectId){
          const nextAmount = Number(data.currentAmount) + Number(weiToEther(amount));
          data["currentAmount"] = nextAmount.toString()
          data["contractBalance"] = (Number(data.contractBalance)+Number(weiToEther(amount))).toString()
          data["progress"]=Math.min(100,Math.round((nextAmount/Number(data.goalAmount))*100))
        }
        return data
      })
      return {
